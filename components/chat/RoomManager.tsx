...
export function RoomManager({ currentRoomId, onRoomSelect, onLobbySelect }: RoomManagerProps) {
  ...
  // Subscribe to direct messages the user participates in
  useEffect(() => {
    if (!user || !db) return

    const dmRef = collection(db, "directMessages")
    const q = query(dmRef, where("participants", "array-contains", user.uid))
    const unsubscribe = onSnapshot(q, snapshot => {
      const dmsData = snapshot.docs.map(doc => {
        const data = doc.data()
        const dm = { id: doc.id, ...data } as DirectMessage
        if (dm.lastMessage?.timestamp) {
          const ts: any = dm.lastMessage.timestamp
  ...
  // Fetch a limited list of users for DM search
  useEffect(() => {
    if (!db) return
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <div className="glass-panel rounded-xl p-6 space-y-default">
        ...
        <Button
          ...
          className={`w-full justify-start btn-glass ${
            currentRoomId === null
              ? "neon-glow bg-indigo-600/50 text-white"
      ...
      {/* Rooms and DMs list */}
      <div className="flex-1 overflow-y-auto p-4 space-y-default">
        {rooms.length > 0 && (
          <div>
            ...
            <div className="space-y-default">
              {rooms.map(room => (
                <div key={room.id} className="flex items-center space-x-default">
                  <Button
                    variant={currentRoomId === room.id ? "secondary" : "ghost"}
                    onClick={() => onRoomSelect(room.id, "room")}
                    className={`flex-1 justify-start glass ${
                  ...
                  >
                    ...
                    {room.isPrivate && <span className="ml-auto text-xs">🔒</span>}
                  </Button>
        ...
        >
          <DialogContent className="glass max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader className="sr-only">
              ...
              <DialogDescription>Manage room members and settings.</DialogDescription>
            </DialogHeader>
            <RoomMemberManager
