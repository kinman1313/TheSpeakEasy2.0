  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, snapshot => {
      ...
      const availableUsers = usersData.filter(
        u => u.id !== user?.uid && !room?.members.includes(u.id)
      )
  ...
  const inviteUser = async (targetUser: User) => {
    ...
    try {
      ...
      const response = await fetch(`/api/rooms/${roomId}/members`, {
        ...
        body: JSON.stringify({ memberId: targetUser.id, action: "add" }),
      })
      if (!response.ok) {
  ...
  const removeUser = async (targetUser: RoomMember) => {
    ...
    try {
      ...
      const response = await fetch(`/api/rooms/${roomId}/members`, {
        ...
        body: JSON.stringify({ memberId: targetUser.id, action: "remove" }),
      })
      if (!response.ok) {
    ...
    <div className="space-y-default">
          ...
          {isOwner && (
            <Dialog open={isInviteDialogOpen} onOpenChange={setIsInviteDialogOpen}>
              ...
              <DialogContent className="glass-card">
                ...
                <div className="space-y-default py-4">
                  ...
                  <div className="max-h-64 overflow-y-auto space-y-default">
                    {filteredUsers.length === 0 ? (
                      ...
                      filteredUsers.map(targetUser => (
                        <div
                          key={targetUser.id}
                          className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-700/50 transition-colors glass-card"
                        >
      ...
      <div className="space-y-default">
        ...
        {isLoading ? (
          ...
          <div className="space-y-default max-h-96 overflow-y-auto">
            {members.map(member => (
              <div
                key={member.id}
      ...
      {/* Footer actions */}
      <div className="flex justify-end space-x-default pt-4 border-t border-slate-700/50">
